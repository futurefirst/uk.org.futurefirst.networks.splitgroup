...
use CRM_Splitgroup_ExtensionUtil as E;
...
/**
 * @return CRM_Splitgroup_Upgrader
 */
function _splitgroup_civix_upgrader() {
  if (!file_exists(__DIR__ . '/CRM/Splitgroup/Upgrader.php')) {
    return NULL;
  }
  else {
    return CRM_Splitgroup_Upgrader_Base::instance();
  }
}
...
function _splitgroup_civix_civicrm_caseTypes(&$caseTypes) {
  ...
  foreach (_splitgroup_civix_glob(__DIR__ . '/xml/case/*.xml') as $file) {
...
function _splitgroup_civix_civicrm_angularModules(&$angularModules) {
  ...
  $files = _splitgroup_civix_glob(__DIR__ . '/ang/*.ang.php');
 ...
 * missing them.
 */
function _splitgroup_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
  ...
  _splitgroup_civix_fixNavigationMenuItems($nodes, $maxNavID, NULL);
}

function _splitgroup_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
  ...
  foreach ($origKeys as $origKey) {
    ...
    if (isset($nodes[$origKey]['child']) && is_array($nodes[$origKey]['child'])) {
      _splitgroup_civix_fixNavigationMenuItems($nodes[$origKey]['child'], $maxNavID, $nodes[$origKey]['attributes']['navID']);
    }
  }
}
